...
export function createCodexRunner(input: {
  ...
  async function runCodexPhase(phaseInput: {
    ...
    phaseLabel: string;
    logger: AppLogger;
  }): Promise<{ ok: true; output: string } | { ok: false; reason: string; cancelled?: boolean }> {
    ...
    try {
      ...
      // Only include output schema for the JSON-validating phase
      ...
      if (phaseInput.abortSignal) {
        ...
        phaseInput.abortSignal.addEventListener("abort", abortListener, { once: true });
    ...
    async review(
      ...
      try {
        ...
        if (!result.success) {
          ...
          return { ok: false, reason: "Codex returned JSON that did not match the review schema." };
        }
        ...
        return { ok: false, reason: "Codex review process could not be started or parsed safely." };
      }
    },
